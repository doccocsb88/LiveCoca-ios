...
@property (nonatomic, strong) GPUImageTransformFilter *transFilter;
...
-(GPUImageTransformFilter *)transFilter{
    if (!_transFilter) {
        _transFilter = [[GPUImageTransformFilter alloc] init];
        _transFilter.affineTransform = CGAffineTransformConcat(CGAffineTransformMakeTranslation(2, 4), CGAffineTransformMakeScale(0.3, 0.3));
...
- (void)reloadFilter{
    ...
    if([self.configuration.avSessionPreset isEqualToString:AVCaptureSessionPreset640x480]){
        ...
        if (self.trans) {
            [self.videoCamera addTarget:self.transFilter];
            [self.transFilter addTarget:self.filter];
